# src/models/model8.py
# ExtraTrees (Extremely Randomized Trees) - 더 높은 랜덤성
import pandas as pd
import numpy as np
...
class BettingModel8:
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        # ExtraTrees with high randomization
        best_params = {
            'n_estimators': 600,               # 많은 트리로 분산 감소
            'max_depth': 10,                   # 더 깊은 트리 허용
            'min_samples_split': 15,           # 분할 최소 샘플
            'min_samples_leaf': 8,             # 리프 최소 샘플
            'max_features': 0.7,               # 70% 특성 사용
            'max_leaf_nodes': 80,              # 최대 리프 노드
        ...
        # 모델 초기화 및 학습
        self.model = ExtraTreesClassifier(**best_params)
        
        print("\n=== Model8 (ExtraTrees) 학습 시작 ===")
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"betting_model8_{timestamp}.joblib"
        feature_path = self.model_dir / f"features8_{timestamp}.json"
