# src/models/model2_with_odds.py
"""
배당 변수를 포함한 CatBoost 모델
- 기존 model2.py 복사본
...
class BettingModel2WithOdds:
    """배당 변수를 포함한 CatBoost 베팅 모델"""
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        model_path = self.model_dir / f"betting_model2_with_odds_{timestamp}.joblib"
        feature_path = self.model_dir / f"features2_with_odds_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {'type': 'catboost_with_odds', 'odds_buckets': 8}
            }, f, indent=2)
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏀 배당 변수 포함 CatBoost 모델 학습 (Model2)")
    print("=" * 70)
    
    data = get_latest_processed_data_with_odds()
    model = BettingModel2WithOdds()
    X, y = model.prepare_features(data)
    
    print(f"\n특성 수: {len(model.feature_names)}, 샘플 수: {len(X)}")
