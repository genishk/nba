# src/models/model7_totals.py
# RandomForest Regressor - Total Score Prediction
import pandas as pd
import numpy as np
...
class BettingModel7Totals:
    """RandomForest 회귀 모델 - 총점 예측"""
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        # RandomForest Regressor with balanced parameters
        best_params = {
            'n_estimators': 500,
            'max_depth': 8,
        ...
        print("\n=== Model7 Totals (RandomForest Regressor) 학습 시작 ===")
        self.model.fit(X, y, sample_weight=sample_weights)
        
        print(f"OOB Score (R²): {self.model.oob_score_:.3f}")
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"totals_model7_{timestamp}.joblib"
        feature_path = self.model_dir / f"totals_features7_{timestamp}.json"
