# src/models/model6_with_odds.py
"""
배당 변수를 포함한 XGBoost Histogram 모델
- 기존 model6.py 복사본
...
class BettingModel6WithOdds:
    """배당 변수를 포함한 XGBoost Histogram 베팅 모델"""
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        print("\n=== Model6 (XGBoost Histogram) 배당 포함 학습 시작 ===")
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        model_path = self.model_dir / f"betting_model6_with_odds_{timestamp}.joblib"
        feature_path = self.model_dir / f"features6_with_odds_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {'type': 'xgboost_hist_with_odds', 'odds_buckets': 8}
            }, f, indent=2)
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏀 배당 변수 포함 XGBoost Histogram 모델 학습 (Model6)")
    print("=" * 70)
    
    data = get_latest_processed_data_with_odds()
    model = BettingModel6WithOdds()
