# src/models/model4.py
# LightGBM v2 - DART boosting with aggressive feature sampling
import pandas as pd
import numpy as np
...
class BettingModel4:
    """LightGBM 모델 v2 - DART boosting, 공격적 특성 샘플링"""
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        """DART boosting 기반 LightGBM 모델 학습"""
        n_samples = len(X)
        # 선형 증가 가중치 (완만한 증가)
        sample_weights = np.linspace(1, 1.5, n_samples)
        
        # DART boosting - 드롭아웃 기반으로 과적합 방지
        best_params = {
            'boosting_type': 'dart',           # DART boosting 사용
            'drop_rate': 0.15,                 # 트리 드롭 비율
            'skip_drop': 0.5,                  # 드롭 스킵 확률
            'max_drop': 50,                    # 최대 드롭 트리 수
            'colsample_bytree': 0.8,           # 더 많은 특성 사용
            'learning_rate': 0.08,             # 중간 학습률
            'max_depth': 6,                    # 더 깊은 트리
            'min_child_samples': 50,           # 안정적인 리프 노드
            'n_estimators': 400,               # 충분한 트리 개수
            'num_leaves': 40,                  # 더 많은 리프 노드
            'reg_alpha': 0.5,                  # 약한 L1 규제
            'reg_lambda': 5.0,                 # 중간 L2 규제
            'subsample': 0.75,                 # 데이터 샘플링
            'subsample_freq': 1,               # 매 반복마다 샘플링
            'random_state': 42,
            'verbose': -1,
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"betting_model4_{timestamp}.joblib"
        feature_path = self.model_dir / f"features4_{timestamp}.json"
