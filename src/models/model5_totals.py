# src/models/model5_totals.py
# CatBoost Ordered Regressor - Total Score Prediction
import pandas as pd
import numpy as np
...
class BettingModel5Totals:
    """CatBoost Ordered 회귀 모델 - 총점 예측"""
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        """Ordered boosting 기반 CatBoost 회귀 모델 학습"""
        n_samples = len(X)
        ...
        print("\n=== Model5 Totals (CatBoost Ordered Regressor) 학습 시작 ===")
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"totals_model5_{timestamp}.joblib"
        feature_path = self.model_dir / f"totals_features5_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {
                    'type': 'catboost_ordered_regressor',
                    'target': 'total_score',
