# src/models/model5.py
# CatBoost v2 - Ordered boosting with symmetric trees
import pandas as pd
import numpy as np
...
class BettingModel5:
    """CatBoost 모델 v2 - Ordered boosting, 대칭 트리, 다른 규제 전략"""
    ...
    def train_model(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        """Ordered boosting 기반 CatBoost 모델 학습"""
        n_samples = len(X)
        # 제곱근 증가 가중치 (중간 정도의 최신 데이터 강조)
        sample_weights = np.sqrt(np.linspace(1, 4, n_samples))
        ...
        best_params = {
            ...
            'depth': 5,                        # 중간 깊이
            'l2_leaf_reg': 4.0,               # 중간 L2 규제
            'bootstrap_type': 'Bayesian',      # Bayesian 부트스트랩
            'bagging_temperature': 0.8,        # 부트스트랩 온도
            'random_strength': 1.5,            # 랜덤 분할 강도
            'border_count': 128,               # 특성 분할 경계 수
            'grow_policy': 'SymmetricTree',    # 대칭 트리 성장
            'min_data_in_leaf': 20,            # 리프 최소 데이터
            'max_leaves': 32,                  # 최대 리프 수
            'early_stopping_rounds': 100,
            'task_type': 'CPU',
            'loss_function': 'Logloss',
            'eval_metric': 'AUC',
            'leaf_estimation_method': 'Gradient',  # Gradient 기반 리프 추정
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        # 모델 파일 경로
        model_path = self.model_dir / f"betting_model5_{timestamp}.joblib"
        feature_path = self.model_dir / f"features5_{timestamp}.json"
