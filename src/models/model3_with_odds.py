# src/models/model3_with_odds.py
"""
배당 변수를 포함한 XGBoost 모델
- 기존 model3.py 복사본
...
class BettingModel3WithOdds:
    """배당 변수를 포함한 XGBoost 베팅 모델"""
    ...
    def save_model(self, timestamp: str = None) -> None:
        ...
        model_path = self.model_dir / f"betting_model3_with_odds_{timestamp}.joblib"
        feature_path = self.model_dir / f"features3_with_odds_{timestamp}.json"
        ...
        with open(feature_path, 'w') as f:
            json.dump({
                ...
                'model_info': {'type': 'xgboost_with_odds', 'odds_buckets': 8}
            }, f, indent=2)
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏀 배당 변수 포함 XGBoost 모델 학습 (Model3)")
    print("=" * 70)
    
    data = get_latest_processed_data_with_odds()
    model = BettingModel3WithOdds()
